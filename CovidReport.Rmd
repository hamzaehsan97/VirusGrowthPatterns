...
trends <- trends %>% filter((trends$hits > 0.0))
```


```{r}
#Create google trends plot
...
country_lat_long<- country_confirm_lat_long[!duplicated(country_confirm_lat_long$Country.Region),]
  ...
  mainPanel( 
    fluidRow(
      leafletOutput(outputId = "mymap",width = "100%", height = 400),
          ...
          checkboxInput("markers", "Cases", FALSE),
          checkboxInput("deaths", "Heatmap", FALSE)
        ),
    ),
    fluidRow(
       column(4, 
...
#create the map
  output$mymap <- renderLeaflet({
    mapday <-   as.numeric(as.character(input$day_number))  
    leaflet(data) %>% 
      setView(lng = 41, lat = 12, zoom = 1.4)  %>% 
      addTiles() %>% 
      addCircles(data = data%>%filter(data$days.x == mapday),
                 lat = ~ data$Lat,
                 lng = ~ data$Long,
                 weight = 1,
                 radius = ~sqrt(data$confirm_cases)*250,
                 popup = ~as.character(data$confirm_cases),
                 label = ~as.character(data$Country.Region.x),
                 color = ~pal(data$confirm_cases),
                 fillOpacity = 0.5
                 )
  })
  ...
  observe({
    proxy <- leafletProxy("mymap", data = data%>%filter(data$days.x == input$day_number))
    proxy %>% clearMarkers()
    if (input$markers) {
      proxy %>% addCircleMarkers(stroke = FALSE,
                                 color = ~pal(data$confirm_cases),
                                 fillOpacity = 0.05,
                                 radius = ~sqrt(data$confirm_cases)*0.1,
                                 label = ~as.character(paste0(data$Country.Region.x,
                                  sep = " ",
                                  data$confirm_cases))
                                 ) %>%
                                  addLegend("bottomright",
                                            pal = pal2,
                                            values = data$confirm_cases,
  ...
  observe({
    ...
    proxy %>% clearMarkers()
    if (input$deaths) {
      proxy %>% addHeatmap(lng=~data$Long,
                            lat=~data$Lat,
                            blur =  10,
                            max = 0.05,
                            radius = ~sqrt(data$death_cases)*1000,
                            minOpacity = 0.5) 
      }
    else{
      proxy %>% clearHeatmap()
